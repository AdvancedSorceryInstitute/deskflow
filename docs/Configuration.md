...
|relativeMouseMoves  | `0`, `1` or `2`   | Controls when secondary computers move the mouse using relative rather than absolute mouse moves. This is intended to make Deskflow work better with certain games, which read raw input and cannot use absolute mouse moves. `0` (default) never sends relative moves, so all mouse moves are absolute. `1` sends relative moves when and only when the cursor is locked to the computer (by ''Scroll Lock'' or a configured hot key). `2` always sends relative moves while the cursor is on a client computer; the server then tracks a virtual cursor position so that switching computers at a screen edge keeps working. With `2` you will likely want ''enableSwitchDelay'' or ''enableSwitchDoubleTap'' as well, so that large in-game mouse movements do not switch computers by accident. For backwards compatibility the legacy values ''false'' and ''true'' are read as `0` and `1`.|
